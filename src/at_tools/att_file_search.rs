...
impl Tool for AttFileSearch {
    async fn tool_execute(&self, ccx: &mut AtCommandsContext, tool_call_id: &String, args: &HashMap<String, Value>) -> Result<Vec<ContextEnum>, (String, Option<ChatUsage>)> {
        let file_path = match args.get("file_path") {
            ...
            Some(v) => return Err((format!("argument `file_path` is not a string: {:?}", v), None)),
            None => return Err(("Missing argument `file_path` for att_file_search".to_string(), None))
        };
        let query = match args.get("query") {
            Some(Value::String(s)) => s.clone(),
            Some(v) => return Err((format!("argument `query` is not a string: {:?}", v), None)),
            None => return Err(("Missing argument `query` for att_file_search".to_string(), None))
        };

        let vector_of_context_file = execute_at_file_search(ccx, &file_path, &query, true).await.map_err(|e|(e, None))?;
