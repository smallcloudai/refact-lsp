...
impl Tool for AttAstLookupSymbols {
    async fn tool_execute(&self, ccx: &mut AtCommandsContext, tool_call_id: &String, args: &HashMap<String, Value>) -> Result<Vec<ContextEnum>, (String, Option<ChatUsage>)> {
        ...
        let line_n = match args.get("line_number") {
            ...
            Some(v) => return Err((format!("argument `line_number` is not a valid u64: {:?}", v), None)),
            None => return Err(("no line_number".to_string(), None)),
        };

        let results = execute_at_ast_lookup_symbols(ccx, &path, line_n.unwrap()).await.map_err(|e|(e, None))?;
