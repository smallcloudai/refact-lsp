...
use crate::vecdb::vdb_highlev::{memories_search, memories_select_all, ongoing_find};
...
impl Tool for AttGetKnowledge {
    async fn tool_execute(&self, ccx: &mut AtCommandsContext, tool_call_id: &String, args: &HashMap<String, Value>) -> Result<Vec<ContextEnum>, String> {
        ...
        let memories = memories_select_all(vec_db.clone()).await?;

        let memories_json = memories.iter().map(|m| {
            serde_json::json!({
                "memid": m.memid.clone(),
