...
impl ScratchpadAbstract for ChatPassthrough {
    ...
    async fn prompt(
        ...
        let top_n: usize = 10;
        let last_user_msg_starts = run_at_commands(self.global_context.clone(), self.t.tokenizer.clone(), sampling_parameters_to_patch.max_new_tokens as i32, context_size as i32, &mut self.post, top_n, &mut self.has_vecdb_results).await;
