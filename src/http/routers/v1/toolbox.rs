...
pub async fn handle_v1_customization(
    ...
    let customization_web = caps.read().unwrap().customization.clone();
    let customization_web_mb = if customization_web.is_empty() {
        None
    } else {
    ...
    let tconfig = match crate::toolbox::toolbox_config::load_customization_high_level(
        ...
        Err(err) => {
            ...
            return Ok(Response::builder()
                ...
                .body(Body::from(
                    serde_json::to_string_pretty(&json!({ "detail": err.to_string() })).unwrap(),
                ))
                .unwrap());
        }
    };
...
pub async fn handle_v1_rewrite_assistant_says_to_at_commands(
    ...
    let post = serde_json::from_slice::<SnippetAcceptedPostData>(&body_bytes)
        ...
        if msg.role == "ignore" {
            ...
            for line in msg.content.lines() {
                if line.starts_with("original-toolbox-command ") {
                    original_toolbox_command =
                        line.replace("original-toolbox-command ", "").to_string();
                }
