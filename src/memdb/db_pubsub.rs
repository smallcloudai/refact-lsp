...
use crate::memdb::db_structs::{Chore, ChoreEvent, MemDB};
...
pub async fn handle_pubsub(
    ...
    let mut last_pubevent_id = _get_last_pubevent_id(
        pubsub_poll(lite.clone(), &post.channel, None)
            .await
    ...
    let (preexisting_items, maybe_obj_ids_to_keep) =
        if let Some(quick_search_query) = post.quick_search {
            let mut preexisting_memories = crate::vecdb::vdb_highlev::memories_select_like(
                vec_db.clone(),
                &quick_search_query,
            )
            .await
            ...
            let mut preexisting_memories =
                crate::vecdb::vdb_highlev::memories_select_all(vec_db.clone())
                    .await
                    .map_err(|e| {
    ...
    let sse = stream! {
        for item in preexisting_items.iter() {
            if let Some(obj_ids_to_keep) = &maybe_obj_ids_to_keep {
                if !obj_ids_to_keep.contains(&item.pubevent_obj_id) {
                    continue;
                }
            }
            ...
            let e = json!({
                ...
                "pubevent_obj_id": item.pubevent_obj_id,
        ...
        loop {
            match pubsub_trigerred(gcx.clone(), memdb.clone(), 5).await {
                Ok(_) => {}
                Err(_) => {
                    break;
                }
            };
            match pubsub_poll(lite.clone(), &post.channel, Some(last_pubevent_id)).await {
                Ok(new_events) => {
