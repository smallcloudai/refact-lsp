...
async fn destroy_expired_sessions(gcx: Arc<ARwLock<GlobalContext>>) {
    let sessions = {
        let gcx_locked = gcx.read().await;
        gcx_locked.command_sessions.iter()
    ...
    for key in expired_keys {
        gcx_locked.command_sessions.remove(&key);
    }
}

pub async fn destroy_expired_sessions_background_task(
    gcx: Arc<ARwLock<GlobalContext>>,
) {
    loop {
        tokio::time::sleep(tokio::time::Duration::from_secs(60)).await;
        destroy_expired_sessions(gcx.clone()).await;
    }
}
