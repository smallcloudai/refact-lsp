...
async fn click_on_point(tab: &Arc<Tab>, point: &Point) -> Result<String, String> {
    ...
    tab.wait_until_navigated().map_err(|e| e.to_string())?;
    Ok(format!("Successfully clicked on `{} {}`", point.x, point.y))
}
        ...
        "click" => {
            match command_args.as_slice() {
                [_, x_str, y_str] => {
                    ...
                    let point = Point { x, y };
                    let content = click_on_point(&tab, &point).await
                        .map_err(|e| { format!("Failed to click on `{} {}`: {}.", x_str, y_str, e) })?;
                    tool_log.push(content);
                },
                _ => {
                    tool_log.push(format!(
                        "Missing one or both location arguments `x` and `y`: {:?}. Call this command another time using format `click x y`.",
                        command_args
                    ));
                }
            }
