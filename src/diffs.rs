...
pub fn read_files_n_apply_diff_chunks(
    ...
    fn process_chunks_edit(
        ...
        for file_name in file_names {
            ...
            let file_text = match crate::files_in_workspace::read_file_from_disk_sync(gcx.clone(), &PathBuf::from(&file_name)) {
                Ok(t) => t.to_string(),
                Err(_) => {
    ...
    process_chunks_edit(gcx.clone(), chunks_apply_edit, chunks_undo_edit, max_fuzzy_n, &mut results, &mut outputs);
    process_chunks_other(chunks_apply_other, chunks_undo_other, &mut results, &mut outputs);
