(function() {
    ...
    open_chat && open_chat.addEventListener("click", () => {
        postMessage({type: "open_chat_in_new_tab"});
    });

    send_chat_to_sidebar && send_chat_to_sidebar.addEventListener("click", () => {
        postMessage({type: "send-chat-to-sidebar"});
    });
    ...
    back_button && back_button.addEventListener('click', () => {
        postMessage({ type: "back-from-chat" });
    });
